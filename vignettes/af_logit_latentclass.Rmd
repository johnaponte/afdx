---
title: "Using the afdx package to fit a latent class model to estimate the attributable fraction"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the afdx package to fit a latent class model to estimate the attributable fraction}
...
we follow the method proposed by Smith et al. (1) and Vountasou et al (2) fitting 
...
## The latent class model
    ...
    * Explanation of the calculation of sensitivity, specificity and predictive values
...
# Define the cutoffs points. Number of categories should be big enough

cutpoints <-c(1,100,200,400,800,1600,3200,6400,12800,25600,51200, 102400, 204800)

#Prepare the data for the model. Categories are defined with the cutpoints
data <- 
  malaria_df1 %>%
...
summary_table <-
  ...
  mutate(cutoff  = c(NA, rep(cutpoints[-length(cutpoints)],4))) %>%
...
## Explore the posterior distribution of the attributable fraction

```{r, fig.width = 8, fig.height=4, out.width= "100%", dpi = 300}
ggmcmc(latent_chains, family = "lambda", height = 4, file = NULL)
```

## Graph of diagnostic characteristics 
